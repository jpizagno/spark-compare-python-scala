...
libraryDependencies ++= Seq(
  //
  // NOTE: using 2 percents %% below will add "_2.11" to your dependences, which might not befound in maven:  use % not %%
  //
  "org.apache.spark"          %% "spark-streaming"      % sparkVersion % clusterDependencyScope,
  "org.apache.httpcomponents" % "httpclient"            % "4.5.6", // needed for httpClient call to Persistence/SpringBoot-REST
  "org.apache.spark"          %% "spark-sql-kafka-0-10" % sparkVersion, // removed "_2.11" because sbt adds a _2.11
...
libraryDependencies ++= Seq(
  ...
  "com.holdenkarau"  %% "spark-testing-base" % "2.3.3_0.12.0",
...
/*
 * needed for ./sbt assembly. without this cached files interfere.
 * avoids ERROR: deduplicate: different file contents found in the following:
 */
...
Compile / mainClass := Some("com.news.analyzer.Main")
mainClass in (Compile, packageBin) := Some("com.news.analyzer.Main")

// gather dpendencies locally
retrieveManaged := true

// Jar name, artifact
assemblyJarName in assembly := "test-scala.jar"
